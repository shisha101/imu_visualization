...
class imu_calibration():
    ...
    def yaml_write(self, name):
        ...
        # simple objects
        output_list_simple = [0]*6
        # iterate on the 3 lists together
        for list_entry, name, value in zip(output_list_simple,name_list, value_list):
            list_entry = self.write_simple_sub(value, name)
...
#         output_list[0] = self.write_sub(self.acc_direction_correction_list, "acc_direction")
#         output_list[1] = self.write_sub(self.acc_vector_mapping_direction_correction_list, "acc_mapping")
#         output_list[2] = self.write_sub(self.gyro_direction_correction_list, "gyro_direction")
#         output_list[3] = self.write_sub(self.gyro_vector_mapping_direction_correction_list, "gyro_mapping")
#         output_list[4] = self.write_sub(self.mag_direction_correction_list, "mag_direction")
#         output_list[5] = self.write_sub(self.mag_vector_mapping_direction_correction_list, "mag_mapping")
        
        #physical dump to file
        with open(self.absolute_path+self.imu_name+"_config.yaml", 'w') as f:
            f.write('# autogenerated: Do not edit #\n')
            f.write('# Complex representation #\n')
            f.write(yaml.dump(output_list))
            f.write('# Simple representation #\n')
            f.write(yaml.dump(output_list_simple))
        x = yaml.load(file(self.absolute_path+self.imu_name+"_config.yaml"))
        print (x)
        y = x
        print (y[3])
